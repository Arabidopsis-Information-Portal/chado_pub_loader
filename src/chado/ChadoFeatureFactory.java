...
public class ChadoFeatureFactory {
	private static final Logger LOG = Logger.getLogger(ChadoFeatureFactory.class);
    ...
    public int getFeatureId (int typeID, String name) throws SQLException {
    	int featureId = 0;
    	String query = "SELECT feature_id FROM feature where is_obsolete = FALSE AND type_id = ? AND uniquename=?";
    ...
    public int getFeaturePubRecord(int featureID, int pubID)throws SQLException {
    	int featurePubID = 0;
    	String query = "SELECT feature_pub_id FROM feature_pub WHERE feature_id=? AND pub_id =?";
    	PreparedStatement stmt = connection.prepareStatement (query);
    ...
    public int setFeaturePubRecord(int featureID, int pubID) throws SQLException {
    	...
    	if(featurePubID==0){
    		String query = "INSERT into feature_pub (feature_id, pub_id) VALUES (?,?)";
    		PreparedStatement stmt = connection.prepareStatement (query, Statement.RETURN_GENERATED_KEYS);
    		stmt.setInt(1, featureID);
    		stmt.setInt(2,pubID);
    		int count = stmt.executeUpdate();
    		if (count > 1){
    			LOG.warn("featureID="+ featureID+", pubID="+pubID);
    			LOG.warn("query:\""+query+"\"");
    			LOG.warn("made " + count + " updates");
    		}
